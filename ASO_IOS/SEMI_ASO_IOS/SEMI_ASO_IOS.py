...
def main(args):
    ...
    try:
        ...
        try:
            dic_landmark = _selected_landmarks(args)
        ...
        dic_gold = {}
        try:
            _load_gold_landmarks(args, dic_gold)
        ...
        try:
            icp = _build_icp_methods(dic_landmark)
        ...
        try:
            list_file = _discover_input_files(args, link)
        ...
        for index, file in tqdm(enumerate(list_file), total=len(list_file)):
            jaw, success_count = _register_one_file(args, dic_gold, error_details, failed_indices, file, icp, index, jaw, link, list_file, success_count)
